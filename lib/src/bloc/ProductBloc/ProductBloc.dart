  ...
  Stream<ProductsState> mapEventToState(event) async* {
    if (event is ProductLoadEvent) {
      ...
      yield Loading(sortBy: state.sortBy, error: null, filterRules: null, data: listdata);
       final response = await http.get("https://mapi.sendo.vn/mob/product/cat/thoi-trang-nu?p=" + currentPage.toString());
       print(response.body);
       listdata=  json.decode(response.body)['data'].cast<Map<String,dynamic>>().map<Product>((json) => Product.fromJson(json)).toList();
       print(listdata);
      currentPage++;
